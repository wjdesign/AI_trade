...
| `STOP_LOSS_PCT` | 3% | 固定止損上限；ATR 過小時保底 2.5% 空間 |
| `SLIPPAGE_LIMIT` | 1% | 最大允許買賣價差（零股市場較大） |
| `MIN_ORDER_VALUE` | 9,000 元 | 最小下單金額（避免手續費侵蝕） |
...
| 帳戶可用資金 | 建議 ≥ `TOTAL_BUDGET`（45,000 元），正式模式下系統會自動讀取實際餘額 |

### 風險參數核對

| 參數 | 目前值 | 說明 |
|------|--------|------|
| `MAX_POSITIONS` | 3 | 最多同時持有 3 檔，單筆 15,000 元 |
| `STOP_LOSS_PCT` | 3% | 每筆最大虧損上限 |
| `MIN_ORDER_VALUE` | 9,000 元 | 低於此金額不進場 |
| `TIME_STOP_MINUTES` | 0 | 目前停用時間停損（波段模式） |

### 已完成的前置修復

- ✅ `chips.py` — TWSE API 回傳 int 型別導致的解析錯誤
- ✅ ATR 過小時止損空間不足（已加入最小 2.5% 保底）
- ✅ `MIN_ORDER_VALUE` 從 10,000 調降至 9,000（2454 聯發科不再被過濾）

### 上線後首日建議

1. 觀察 Telegram 推播是否正常（買進/賣出通知）
2. 確認 `logs/trades_YYYYMMDD.csv` 有正確記錄成交紀錄
3. 首日建議以 1 檔部位（`MAX_POSITIONS=1`）試跑，確認整個下單流程無誤後再恢復 3 檔
